import { useEffect, useState } from 'react';
import { useNavigate } from 'react-router-dom';
import api from '../api';

export default function ProveedoresLista() {
  const [proveedores, setProveedores] = useState([]);
  const [cargando, setCargando] = useState(true);
  const navigate = useNavigate();

  useEffect(() => {
    obtenerProveedores();
  }, []);

  const obtenerProveedores = async () => {
    try {
      const res = await api.get('/proveedores');
      setProveedores(res.data);
    } catch (error) {
      console.error('Error al obtener proveedores:', error);
    } finally {
      setCargando(false);
    }
  };

  const eliminarProveedor = async (id) => {
    const confirmar = confirm('¿Estás seguro que deseas eliminar este proveedor?');
    if (!confirmar) return;

    try {
      await api.delete(`/proveedores/${id}`);
      setProveedores(proveedores.filter((p) => p._id !== id));
    } catch (error) {
      alert('Error al eliminar proveedor.');
    }
  };

  const [filtros, setFiltros] = useState({
    nombre: '',
    estado: '',
  });

  useEffect(() => {
    const timeout = setTimeout(() => {
      buscarConFiltros();
    }, 300);

    return () => clearTimeout(timeout);
  }, [filtros]);

  const buscarConFiltros = async () => {
    const query = new URLSearchParams(filtros).toString();
    const res = await fetch(`${import.meta.env.VITE_API_URL}/proveedores?${query}`);
    const data = await res.json();
    setProveedores(data);
  };

  return (
    <>
      <div className="flex justify-between items-center mb-4">
        <h1 className="text-2xl font-bold">Proveedores Registrados</h1>
        <button
          onClick={() => navigate('/proveedores/nuevo')}
          className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-md text-sm font-medium"
        >
          + Agregar Proveedor
        </button>
      </div>

      <div className="overflow-x-auto">
        <table className="min-w-full bg-white border border-gray-200 shadow-sm rounded-md">
          <thead className="bg-gray-100 text-gray-700 text-sm">
            <tr>
              <th className="px-4 py-2 text-left">Nombre</th>
              <th className="px-4 py-2 text-left">Teléfono</th>
              <th className="px-4 py-2 text-left hidden md:table-cell">Contacto</th>
              <th className="px-4 py-2 text-left hidden md:table-cell">Correo</th>
              <th className="px-4 py-2 text-left hidden md:table-cell">NRC</th>
              <th className="px-4 py-2 text-left hidden md:table-cell">Estado</th>
              <th className="px-4 py-2 text-center">Acciones</th>
            </tr>
            <tr>
              <th className="px-4 py-1">
                <input
                  type="text"
                  placeholder="Buscar nombre"
                  value={filtros.nombre || ''}
                  onChange={(e) => setFiltros({ ...filtros, nombre: e.target.value })}
                  className="w-full border rounded px-2 py-1 text-sm"
                />
              </th>
              <th className="px-4 py-1 hidden md:table-cell"></th>
              <th className="px-4 py-1 hidden md:table-cell"></th>
              <th className="px-4 py-1 hidden md:table-cell"></th>
              <th className="px-4 py-1 hidden md:table-cell"></th>
              <th className="px-4 py-1 hidden md:table-cell">
                <select
                  value={filtros.estado || ''}
                  onChange={(e) => setFiltros({ ...filtros, estado: e.target.value })}
                  className="w-full border rounded px-2 py-1 text-sm"
                >
                  <option value="">Todos</option>
                  <option value="activo">Activo</option>
                  <option value="inactivo">Inactivo</option>
                </select>
              </th>
              <th></th>
            </tr>
          </thead>
          <tbody className="text-sm text-gray-800">
            {cargando ? (
              <tr>
                <td colSpan="7" className="text-center text-gray-500 py-10">
                  Cargando proveedores...
                </td>
              </tr>
            ) : proveedores.length === 0 ? (
              <tr>
                <td colSpan="7" className="text-center text-gray-500 py-10">
                  No hay proveedores registrados.
                </td>
              </tr>
            ) : (
              proveedores.map((proveedor, i) => (
                <tr key={proveedor._id} className={i % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                  <td className="px-4 py-2 font-medium">
                    <button
                      onClick={() => navigate(`/proveedores/detalle/${proveedor._id}`)}
                      className="text-blue-600 hover:underline"
                    >
                      {proveedor.nombre}
                    </button>
                  </td>
                  <td className="px-4 py-2">{proveedor.telefono}</td>
                  <td className="px-4 py-2 hidden md:table-cell">{proveedor.contacto || '-'}</td>
                  <td className="px-4 py-2 hidden md:table-cell">{proveedor.email || '-'}</td>
                  <td className="px-4 py-2 hidden md:table-cell">{proveedor.nrc}</td>
                  <td className="px-4 py-2 hidden md:table-cell">
                    <span
                      className={`inline-block px-2 py-1 rounded-full text-xs font-bold ${
                        proveedor.estado === 'activo'
                          ? 'bg-green-100 text-green-700'
                          : 'bg-red-100 text-red-700'
                      }`}
                    >
                      {proveedor.estado.toUpperCase()}
                    </span>
                  </td>
                  <td className="px-4 py-2 text-center">
                    <div className="flex flex-col md:flex-row justify-center gap-2">
                      <button
                        onClick={() => navigate(`/proveedores/editar/${proveedor._id}`)}
                        className="text-sm bg-yellow-400 hover:bg-yellow-500 text-white px-3 py-1 rounded"
                      >
                        Editar
                      </button>
                      <button
                        onClick={() => eliminarProveedor(proveedor._id)}
                        className="text-sm bg-red-500 hover:bg-red-600 text-white px-3 py-1 rounded"
                      >
                        Eliminar
                      </button>
                    </div>
                  </td>
                </tr>
              ))
            )}
          </tbody>
        </table>
      </div>
    </>
  );
}
